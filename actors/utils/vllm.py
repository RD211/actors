...
def to_vllm_lora_state_dict(state_dict):
    ...
    qkv_lora_pat = re.compile(r"(.+)\.self_attn\.(q|k|v)_proj\.(lora_[AB])\.([^.]+)\.weight$")
    gate_up_lora_pat = re.compile(r"(.+)\.mlp\.(gate|up)_proj\.(lora_[AB])\.([^.]+)\.weight$")
    ...
    for k, v in state_dict.items():
        ...
        if qkv_match:
            ...
            if len(bucket) == 3:
                ...
                out_sd[f"{prefix}.self_attn.qkv_proj.{lora_type}.{adapter_name}.weight"] = qkv_list
        ...
        if gate_up_match:
            ...
            if len(bucket) == 2:
                ...
                out_sd[f"{prefix}.mlp.gate_up_proj.{lora_type}.{adapter_name}.weight"] = gate_up_list
