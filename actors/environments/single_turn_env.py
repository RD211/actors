...
from actors.rewards.base import RewardFunction, BaseRewardFunction
...
class SimpleSingleTurnEnvironment(Environment):
    ...
    def __init__(
        ...
        for rf in reward_functions:
            if isinstance(rf, BaseRewardFunction):
                ...
                self.reward_functions.append(RewardFunction(
                    name=func_name,
                    weight=1.0,
                    func=rf
                ))
    ...
    async def generate(self, batch: Dict[str, Any]) -> EnvironmentOutput:
        ...
        for text in generated_texts:
            tokenized = self.tokenizer(text, return_tensors="pt", padding=False, truncation=False)
        ...
        if self.mask_prompt_for_loss:
            ...
            for i, (prompt, completion) in enumerate(zip(prompts, completions)):
                prompt_tokens = self.tokenizer(prompt, return_tensors="pt", padding=False, truncation=False).input_ids.squeeze(0).tolist()
        ...
        for i, (prompt, completion) in enumerate(zip(prompts, completions)):
            ...
            for reward_func in self.reward_functions:
                reward_value = reward_func.compute_reward(
                    ...
                    **{k: v[i] for k, v in batch.items() if k != self.prompt_column and type(v) is list}
                )
